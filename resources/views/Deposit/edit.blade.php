@extends('layouts.app') @section('content')
    <div class="container">
        ...
        <div class="row justify-content-center">
            ...
            <form class="smn-form" id="deposit-create-form" method="post" action="./../editRegisterDeposit"
                ...
                <div class="form-group">
                    ...
                    <input type="date" class="form-control " id="deposit_company_payment_date"
                        name="data[Deposit][payment_date]" tabindex="2" value="{{ $depositDatas->payment_date }}">
                </div>
                ...
                <div class="search-area">
                    <div class="sale-date-form">
                        ...
                        <input type="date" class="form-control width-45 sale_from_date" id="sale_from_date"
                            name="data[Deposit][sale_from_date]" onchange='javascript:changeCalcFlg()' tabindex="3"
                            value="{{ $depositDatas->sale_from_date }}">
                        <p class="sale-block">〜</p>
                        <input type="date" class="form-control width-45 sale_to_date" id="sale_to_date"
                            name="data[Deposit][sale_to_date]" onchange='javascript:changeCalcFlg()' tabindex="4"
                            value="{{ $depositDatas->sale_to_date }}">
                    </div>

                    <div class="form-group" style="margin-bottom: 0 !important;">
                        <div class="radio_box">
                            <label><input type="radio" name="data[Deposit][invoice_output_type]" value="0"
                                    onchange="toggleCompanyInput()" @if ($targetType == 'owner') checked @endif>
                                本部企業毎</label>
                            <label><input type="radio" name="data[Deposit][invoice_output_type]" value="1"
                                    onchange="toggleCompanyInput()" @if ($targetType == 'sale') checked @endif>
                                売上先店舗毎</label>
                        </div>
                    </div>
                    ...
                    <button id="calc-btn" class="calc-btn btn btn-primary" type="button"
                        onclick="javascript:calcSalePrice()">計算</button>
                ...
                <div id="result-area" class="result-area">
                    ...
                    <table class="result-table" onchange="javascript:changeCalcFlg()">
                        ...
                        @foreach ($saleSlipDatas as $saleSlipData)
                            ...
                            ?>
                            <tr>
                                <td><input type="checkbox" class="checkbox_list"
                                        id="sale-slip-id-{{ $saleSlipData->id }}"
                                        name="data[DepositDetail][{{ $saleSlipData->id }}][id]"
                                        value="{{ $saleSlipData->id }}" <?php echo $checked; ?>></td>
                                <td>{{ $saleSlipData->date }}
                                    <input type="hidden" name="data[DepositDetail][{{ $saleSlipData->id }}][date]"
                                        value="{{ $saleSlipData->date }}">
                                </td>
                                <td>
                                    <?php echo number_format($notaxSubTotal8); ?>
                                    <input type="hidden" id="sale-slip-subTotal8-{{ $saleSlipData->id }}"
                                        name="data[DepositDetail][{{ $saleSlipData->id }}][notax_subTotal_8]"
                                        value="{{ $notaxSubTotal8 }}">
                                </td>
                                <td>
                                    <?php echo number_format($tax8); ?>
                                    <input type="hidden" id="sale-slip-tax8-{{ $saleSlipData->id }}"
                                        name="data[DepositDetail][{{ $saleSlipData->id }}][tax8]"
                                        value="{{ $tax8 }}">
                                </td>
                                <td>
                                    <?php echo number_format($notaxSubTotal10); ?>
                                    <input type="hidden" id="sale-slip-subTotal10-{{ $saleSlipData->id }}"
                                        name="data[DepositDetail][{{ $saleSlipData->id }}][notax_subTotal_10]"
                                        value="{{ $notaxSubTotal10 }}">
                                </td>
                                <td>
                                    <?php echo number_format($tax10); ?>
                                    <input type="hidden" id="sale-slip-tax10-{{ $saleSlipData->id }}"
                                        name="data[DepositDetail][{{ $saleSlipData->id }}][tax10]"
                                        value="{{ $tax10 }}">
                                </td>
                                <td>
                                    <?php echo number_format($subTotal); ?>
                                    <input type="hidden" name="data[DepositDetail][{{ $saleSlipData->id }}][subTotal]"
                                        value="{{ $subTotal }}">
                                </td>
                                <td>
                                    <?php echo number_format($delivery_price); ?>
                                    <input type="hidden" id="sale-slip-deliveryPrice-{{ $saleSlipData->id }}"
                                        name="data[DepositDetail][{{ $saleSlipData->id }}][delivery_price]"
                                        value="{{ $delivery_price }}">
                                </td>
                                <td>
                                    <?php echo number_format($adjust_price); ?>
                                    <input type="hidden" id="sale-slip-adjustPrice-{{ $saleSlipData->id }}"
                                        name="data[DepositDetail][{{ $saleSlipData->id }}][adjust_price]"
                                        value="{{ $adjust_price }}">
                                </td>
                                <td id="sale-slip-total{{ $saleSlipData->id }}" data-value="{{ $total }}">
                                    <?php echo number_format($total); ?>
                                    <input type="hidden" name="data[DepositDetail][{{ $saleSlipData->id }}][total]"
                                        value="{{ $total }}">
                                </td>
                                <td>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" class="form-control staff_code_input" id="staff_code"
                                name="data[Deposit][staff_code]" value="{{ $depositDatas->staff_code }}" tabindex="7">
                            <input type="hidden" id="staff_id" name="data[Deposit][staff_id]"
                                value="{{ $depositDatas->staff_id }}">
                        </td>
                        <td class="width-30">
                            <input type="text" class="form-control" id="staff_text" name="data[Deposit][staff_text]"
                                value="{{ $depositDatas->staff_name }}" readonly>
                        </td>
                    </tr>
                    <tr>
                        <th>支払金額</th>
                        <td class="width-20">
                            <input type="number" class="form-control" id="price" name="data[Deposit][price]"
                                onchange='javascript:calcTotalSalePrice()' value="{{ $depositDatas->sub_total }}"
                                readonly>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="width-20">
                            <input type="number" class="form-control" id="adjustment_price"
                                name="data[Deposit][adjustment_price]" onchange='javascript:calcTotalSalePrice()'
                                value="{{ $depositDatas->adjustment_amount }}" tabindex="8">
                        </td>
                    </tr>
                    <tr>
                        <th>振込先</th>
                    ...
                    <tr>
                        ...
                        <td colspan="5">
                            <textarea class="form-control" id="memo" name="data[Deposit][memo]" row="5" tabindex="10"
                                style="margin-top: 0px; margin-bottom: 0px; height: 150px;">@php print $depositDatas->remarks @endphp</textarea>
                        </td>
                    </tr>
                </table>
                ...
                <table class="total-table">
                    ...
                    <tr>
                        <td colspan="2"><input type="number" class="form-control" id="total_price"
                                name="data[Deposit][total_price]" value="{{ $depositDatas->amount }}" readonly></td>
                    </tr>
                </table>
                    ...
                    <tr>
                        <td class="width-20">
                            <input type="tel" class="form-control" id="deposit_submit_type"
                                name="data[Deposit][deposit_submit_type]"
                                value="{{ $depositDatas->deposit_submit_type }}">
                        </td>
                        <?php
                        $text = '';
                        if ($depositDatas->deposit_submit_type == 0) {
                            $text = '未入金';
                        }
                        if ($depositDatas->deposit_submit_type == 1) {
                            $text = '入金済';
                        }
                        if ($depositDatas->deposit_submit_type == 2) {
                            $text = '繰越';
                        }
                        ?>
                        <td class="width-30">
                            <input type="text" class="form-control" id="deposit_submit_type_text"
                                name="data[Deposit][deposit_submit_type_text]" value="{{ $text }}" readonly>
                        </td>
                        <td class="width-50">
