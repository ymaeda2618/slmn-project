                    ...
                    <tbody>
                        <tr>
                            <td>
                                ...
                                <div class="table-td">
                                    <input type="date" class="search-control" id="expense_billing_date_to" name="data[ExpenseBilling][expense_billing_date_to]" value="{{$condition_date_to}}" tabindex="2">
                                </div>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <div class="table-th">仕入先先企業</div>
                                <div class="table-td table-code-td">
                                    <input type="text" class="search-control expense_billing_company_code_input" id="expense_billing_company_code" name="data[ExpenseBilling][expense_billing_company_code]" value="{{$condition_company_code}}" tabindex="4">
                                    <input type="hidden" id="expense_billing_company_id" name="data[ExpenseBilling][expense_billing_company_id]" value="{{$condition_company_id}}">
                                </div>
                                <div class="table-td table-name-td">
                                    <input type="text" class="search-control" id="expense_billing_company_text" name="data[ExpenseBilling][expense_billing_company_text]" value="{{$condition_company_text}}" readonly>
                                </div>
                                <div class="table-th">担当者</div>
                                <div class="table-td table-code-td">
                                    <input type="text" class="search-control expense_billing_staff_code_input" id="expense_billing_staff_code" name="data[ExpenseBilling][expense_billing_staff_code]" value="{{$condition_staff_code}}" tabindex="4">
                                    <input type="hidden" id="expense_billing_staff_id" name="data[ExpenseBilling][expense_billing_staff_id]" value="{{$condition_staff_id}}">
                                </div>
                                <div class="table-td table-name-td">
                                    <input type="text" class="search-control" id="expense_billing_staff_text" name="data[ExpenseBilling][expense_billing_staff_text]" value="{{$condition_staff_text}}" readonly>
                                </div>
                            </td>
                        </tr>
                    </tbody>
                ...
                <table class='index-table'>
                    <tbody>
                        <tr>
                            ...
                            <th class="width-10">請求日付</th>
                            <th class="width-10">支払期限</th>
                            <th class="width-20">仕入先企業</th>
                            <th class="width-20">経費種別</th>
                            <th class="width-20">経費名</th>
                            <th class="width-15">担当者</th>
                            <th class="width-10">金額</th>
                            @if (Home::authClerkCheck())
                            <th class="width-10">編集</th> @endif
                        </tr>
                    </tbody>
                </table>
                @foreach ($expenseBillingList as $expense_billing)
                <table class="index-table">
                    <tbody>
                        <tr>
                            @if ($expense_billing->submit_type == 0)
                            <td class="regis-temp width-10">未支払</td>
                            @elseif ($expense_billing->submit_type == 1)
                            <td class="regis-complete width-10">支払済</td>
                            @elseif ($expense_billing->submit_type == 2)
                            <td class="regis-carry width-10">繰越</td>
                            @endif
                            <td>{{$expense_billing->date}}</td>
                            <td class="width-20">{{$expense_billing->due_date}}</td>
                            <td class="width-15">{{$expense_billing->supply_company_name}}</td>
                            <td class="width-15">{{$expense_billing->expense_item_name}}</td>
                            <td class="width-15">{{$expense_billing->name}}</td>
                            <td class="width-15">{{$expense_billing->staff_company_name}}</td>
                            <td>{{number_format($expense_billing->price)}}</td>
                            @if (Home::authClerkCheck())
                            <td><a class='edit-btn' href='./ExpenseBillingEdit/{{$expense_billing->id}}'>編集</a></td> @endif
                        </tr>
                    </tbody>
                </table>
                @endforeach
...
<script type="text/javascript">
    (function($) {
        jQuery(window).load(function() {
            ...
            //-------------------------------------
            $(".expense_billing_company_code_input").autocomplete({
            ...
            $(document).on("blur", "input", function(event) {
                ...
                if (selector_code.match(/expense_billing_company/)) { // 仕入先企業
