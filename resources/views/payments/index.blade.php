...
<div class="container">
    ...
    <div class="row justify-content-center">
        ...
        <div class="top-title">売掛入金一覧</div>

        <!--検索エリア-->
        <div class='search-area'>


            <form id="index-search-form" method="post" action="{{ route('payments.index') }}" enctype="multipart/form-data">
                {{ csrf_field() }}
                <table>
                    <tbody>
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="table-td table-code-td">
                                    <select class="search-control" id="closing_date" name="data[closing_date]">
                                        @foreach ($closing_date_list as $closing_date_key => $closing_date_val)
                                            <option value="{{ $closing_date_key }}" {{ $closing_date_key == $condition_closing_date ? 'selected' : '' }}>
                                                {{ $closing_date_val }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <input type="hidden" id="closing_date_selected" value="{{ $condition_closing_date }}">
                                </div>
                            </td>
        ...
        <div class="accordion">
            ...
            <div class="accordion-content">
                ...
                <div class="csv-btn-area">
                    <a href="#" id="csvDownloadLink">
                        <input type='button' class='search-btn btn-primary' value='CSVダウンロード'>
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class='list-area'>
            <table class='index-table '>
                <tbody>
                    ...
                    @foreach ($paymentReconciliationList as $paymentReconciliation)
                    <tr>
                        <td>{{$paymentReconciliation->company_name}}</td>
                        <td>{{number_format($paymentReconciliation->tax_total)}}</td>
                        <td>{{number_format($paymentReconciliation->accounts_receivable_payment_amount)}}</td>
                        <td>{{number_format($paymentReconciliation->difference)}}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="d-flex justify-content-center">
            {{ $paymentReconciliationList->links() }}
        </div>
    ...
    (function($) {
        jQuery(window).load(function() {
            ...
            $(document).on("blur", "input", function(event) {
                ...
                if (selector_code.match(/company/)) { // 売上先店舗
